...
단순히 성능 최적화를 위한 작업으로 객제 비교 오버헤드가 큰 경우 위력을 발휘한다.
#### 8.5.2 instanceof 연산자를 사용하여 인자의 자료형이 정확한지 검사하라.
동일한 자료형이 아닌라면 false를 반환한다. 만약 클래스가 구현하는 인터페이스 중 하나와 instanceof 연산을 사용해야 할 때에는 해당 인터페이스의 equals규약이 해당 인터페이스를 구현하는 다른 클래스들과 비교가 가능하도록 잘 정돈되어 있는지 확인하고 사용해야한다.
#### 8.5.3 equals의 인자를 정확한 자료형으로 반환하라
앞에 instanceof로 비교를 한 상태이기 때문에 형변환이 실패하지 않는다.
#### 8.5.4 "중요" 필드 각각이 인자로 주어진 객체의 해당 필드와 일치하는지 검사한다.
기본 자료형은 == 연산자로 비교할 수 있지만 그밖에 객체들은 equals를 재귀적으로 호출하며 각각의 필드를 비교해야 한다. 이때 null을 허용하는 필드를 잘 처리해야하는데 아래 코드를 숙지해두자.
```java
field == null ? o.field == null : field.equals(o.field);
// 만약 field와 o.fiedl가 같을 때가 더 많다면 아래 코드가 더 빠르다.
field == o.field || (field != null && field.equals(o.field));
```
만약 각각의 필드를 비교하기 까다로운 경우 완전일치 비교가 가능한 대표형태(canonical form)으로 필드를 변경해서 비교하는 방법도 있다. 이 방법은 대체로 변경 불가능클래스에 적합하며 변경 가능 객체일 경우 객체와 함께 대표 형태도 변경해 주어야한다.
equals 메서드의 성능은 비교 순서와 비교 항목에도 영향을 받을 수 있다.
상황에 따라 비교 비용이 높은 혹은 낮은 필드를 먼저 비교하고 중요 데이터로 부터 파생되는 중복 필드(redundant field)를 비교해 성능을 향항시킬 수도 있다.
#### 8.5.5 equals 메서드 구현을 끝냈다면, 대칭성, 추이성, 일관성의 세 속성이 만족되는지 검토하라.
단순히 검토만 하지말고 단위 테스트로 검사한 후 원인을 파악해내는 것이 중요하다.
#### 8.5.6 기타
- equals를 구현할 때는 hashCode도 재정의 하라
- 너무 머리 쓰지마라
	-필드만 보고 동치성 검사를 하면 규약을 준수하기 수월하다. 하지만 이 경우 앨리어싱(aliasing)까지 동일한 객체로 판단될 수 있는 문제가 있어 주의가 필요하다.
- equals 메서드의 인자형을 object에서 다른 것으로 바꾸지 마라.
	- 성능에서 약간의 이점을 볼 수 있지만 실수로 인한 문제를 찾기 어려울 수 있다. 평소에 @Override 어노테이션을 쓰는 습관을 들이면 문제를 피할 수 있다.
